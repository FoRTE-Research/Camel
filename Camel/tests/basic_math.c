...
#error No checking for CRC_ON or CRC_OFF right now
...
void task1(){
  ...
  GV(arr1)[0] = GV(v1);           // arr1[0] = 2
}

#define writes_task2() cpaso(arr2, 1); cpaso(arr1, 4)
void task2(){
  GV(arr2)[1] = GV(arr1)[0];      // arr2[1] = 2
  GV(arr1)[4] += GV(arr1)[3] + 5; // arr1[4] = 5
}
...
void task3(){
  GV(v3) = GV(v1) + 1;                      // v3 = 3
  GV(arr2)[GV(v3)] = GV(arr1)[GV(v3) + 1];  // arr2[3] = arr1[4] = 5
  // Kills camel!
